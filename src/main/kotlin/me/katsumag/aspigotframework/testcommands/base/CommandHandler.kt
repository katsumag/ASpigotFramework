...
import me.katsumag.aspigotframework.testcommands.annotations.*
...
class CommandHandler internal constructor(// Handler for the parameter types
    ...
    private fun checkOptionalParam(method: Method, command: CommandBase, subCommand: CommandData) {
        ...
        for (i in method.parameters.indices) {
            val parameter = method.parameters[i]
            if (i != method.parameters.size - 1 && parameter.isAnnotationPresent(me.katsumag.aspigotframework.testcommands.annotations.Optional::class.java)) throw CommandException("Method $method.name in class $command.javaClass.name - Optional parameters can only be used as the last parameter of a method!")
            if (parameter.isAnnotationPresent(me.katsumag.aspigotframework.testcommands.annotations.Optional::class.java)) subCommand.optional = true
        }
    }
